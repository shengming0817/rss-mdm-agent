...
test("submit waits for native acceptance; deltas and terminal retain prompt identity", async () => {
  const h = harness(),
    b = await create(h),
    c = fixtureCommand();
  const submitted = await h.adapter.dispatch(
    b,
    c,
    fixtureAttempt(b, c),
    budget(),
  ...
  assert.equal(
    (await h.adapter.dispatch(b, c, fixtureAttempt(b, c), budget())).certainty,
    "submitted",
  );
...
test("ambiguous send and cancellation cannot manufacture a terminal", async () => {
  const h = harness({ acknowledge: false }),
    ...
    c = fixtureCommand();
  assert.equal(
    (await h.adapter.dispatch(b, c, fixtureAttempt(b, c), budget(15)))
      .certainty,
    "unknown",
  );
  const cancel = {
  ...
  assert.equal(
    unwrap(await acknowledge(h.adapter, b, cancel, budget())),
    "request_only",
  );
...
test("two native callbacks under one prompt are answered independently and only once", async () => {
  const h = harness(),
    b = await create(h),
    c = fixtureCommand(),
    sent = await h.adapter.dispatch(b, c, fixtureAttempt(b, c), budget()),
    live = sent.binding;
  const questions = {
    questions: [
      {
  ...
  assert.equal((await acknowledge(h.adapter, live, reply, budget())).ok, true);
...
for (const loss of ["abort", "expiry", "close"])
  test(`pending callback rejects response after ${loss}`, async () => {
    ...
    assert.equal(
      (await acknowledge(h.adapter, live, response, budget())).ok,
      false,
    );
...
test("question response cannot carry permission or cross-account authority", async () => {
  ...
  assert.equal(
    (await acknowledge(h.adapter, live, command, budget())).error.code,
    "invalid_input",
  );
...
test("exhausted budgets do not send prompts or interrupts", async () => {
  ...
  const sent = await h.adapter.dispatch(
    binding,
    cmd,
    fixtureAttempt(binding, cmd),
    budget(),
  ...
  assert.equal(
    (
      await acknowledge(h.adapter, sent.binding, cancel, {
        timeoutMs: 100,
        signal: abort.signal,
      })
    ).ok,
...
test("late native acceptance reconciles unknown submission to submitted", async () => {
  const h = harness({ acknowledge: false }),
    binding = await create(h),
    cmd = fixtureCommand();
  assert.equal(
...
for (const [patch, outcome] of [
  ...
  test(`native terminal maps to the closed ACP-compatible outcome: ${JSON.stringify(patch)}`, async () => {
    ...
    try {
      ...
      assert.equal("queue" in session.capabilities, false);
      const sent = await h.adapter.dispatch(
        session.binding,
        fixtureCommand(),
...
test("dispatch and reconciliation cannot substitute another attempt or namespace", async () => {
  ...
  try {
    assert.equal(
      (await h.adapter.dispatch(binding, command, attempt, budget())).certainty,
      "submitted",
    );
    assert.equal(
      (
