...
客户端 `SessionView.status` 独立保留快照的 active/recovery_required/retired 状态，`connection` 只表示传输附着及同步状态。完整 restore 后即使 attached，恢复不可用的会话仍显示 recovery_required；历史恢复事件不能覆盖较新快照中的健康状态。
