...
| 能力 | 映射与边界 |
| --- | --- |
| 提交 | Host 先持久化 intent，再调用 `submit(binding, command, attempt, budget)`。原生 requestId 是 namespace/session/command/attemptId 的 SHA-256，accepted 映射 `submitted`，不代表耐久完成 |
