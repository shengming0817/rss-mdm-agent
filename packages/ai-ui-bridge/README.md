...
action 携带固定 commandId、expiry、surface revision、interaction/run/generation，失败 `retry()` 复用同一请求。挂载组件暴露 `retry` 与 `remount`。模型内容及回答只是不可信数据，不构成可信计划或执行批准。独立 Vue consumer 的四种操作、恢复与真实输入回传通过 `pnpm check:ai-access-consumer` 验证。
