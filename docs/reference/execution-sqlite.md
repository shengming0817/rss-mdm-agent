...
PR #1038 修复另查阅 rusqlite 0.32.1 的 `src/row.rs`（`Row::get/get_ref`）与同版 `src/transaction.rs`，确认 BLOB 复制与 commit 错误边界；SQLite bundled 源码 `sqlite3.c::lengthFunc/OP_Column` 和官方 [length](https://www.sqlite.org/lang_corefunc.html#length)、[CASE](https://www.sqlite.org/lang_expr.html#the_case_expression) 说明支持从 BLOB 头部读取长度并短路拒绝 payload。采用同 statement 的有界 SQL 投影，无增量句柄或新依赖。

投递确认语义参考 [KafkaConsumer.java](https://github.com/apache/kafka/blob/trunk/clients/src/main/java/org/apache/kafka/clients/consumer/KafkaConsumer.java) 对已处理位置与已获取位置的区分；本实现用逐事件持久确认并直接移除外部游标。错误分类参考 [SQLx error.rs](https://github.com/launchbadge/sqlx/blob/main/sqlx-core/src/error.rs) 区分 Configuration/InvalidArgument 的做法，按本 crate 的三种提交恢复入口建立闭集错误，不复制上游源码。
