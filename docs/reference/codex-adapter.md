...
本地模型传输不证明真实模型身份，一个平台的行为不外推其它平台。操作见[adapter 指南](../../packages/ai-adapters/codex/README.md)。
...
真实模型 smoke 参考 [OpenAI Responses 回执](https://developers.openai.com/api/reference/typescript/resources/beta/subresources/responses/methods/create) 的 response.completed/model/id/status，配置 endpoint 返回的 model/id/status 作为该 endpoint 的回执，不以请求模型名或模型回答自报身份。当前 smoke 的 backendIdentityVerified 仍为 false，不能把自定义网关回执称为官方后端身份验证。未提供明确凭据时 real_model 为 not_run；本地替身的 nonce 连续性正反例只证明协议与脚本判定。
