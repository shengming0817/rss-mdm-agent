...
4. 处理 `resync_required` 时调用 `restore`。分页 token 是有期限且绑定 caller/查询的不可猜测凭据，仅为读视图定位，不授予身份。内存 double 默认30秒/最多128个读视图，未签发 continuation 的完成单页立即释放配额，已签发的多页视图保留到 TTL 支持 token 重试；单页最多256条且同时受总字节/文本/深度/节点预算约束。
