...
获准普通用户打开桌面设置中的“本机安全服务”，应显示已连接/statusOnly。运行 `node scripts/verify-local-service.mjs`；verifier 只读取管理员保护的安装 policy，并校验其中绑定的源码 SHA、桌面与负向 probe 路径/摘要，再执行正向查询、独立进程负例与再次正向查询。调用者不能另传程序路径；任一步失败都不能标记通过，其它攻击场景仍单独验收。
