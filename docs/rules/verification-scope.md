...
- 核心T1：类型、确定性输入/时钟、未知值、预算、授权与状态转换，通过实际调用验证公共 API。
...
功能实施由实际owner建立本仓本地入口，编辑循环运行受影响的最小有效检查；收尾运行本仓`make ci CI_BASE=origin/develop`，一次收集全部失败再集中修复。
不运行父仓CI替代产品验证，不新增远端CI。不存在的入口如实说明，不为文档变更引入工程空壳。
验证记录实际配置/运行模式、命令、结果和未覆盖项；取消、进程退出、安装成功和状态核实分别记录。

## 本地 CI 影响范围

`make ci` 在任务分支比较 `CI_BASE`（默认 `origin/develop`）的 merge-base 到当前工作树（包含已暂存、未暂存与未忽略新文件），
...
必须使用 package.json 指定的 Node 与 pnpm 版本。基线不可用时保守选择全量，已提交状态不作为通过条件。
...
`selection.json`，执行结果原子写入 `latest.json`，带选择原因、范围、命令、实际状态及可得的 HEAD 信息。
