...
fn every_valid_security_leaf_is_covered_by_the_digest() {
    fn changes(v: &serde_json::Value, path: String, out: &mut Vec<(String, serde_json::Value)>) {
        ...
        match v {
            ...
            Value::String(s) if !path.ends_with("/kind") && !path.ends_with("/platform") => {
                let replacement = if path.ends_with("/artifactEncoding") {
                    "utf8Bom".to_owned()
    ...
    assert!(cases
        .iter()
        .any(|(p, _)| p == "/launch/interpreter/artifact/sha256"));
    assert!(cases
        .iter()
    ...
    modified["launch"]["argv"] = serde_json::json!([{"kind":"artifactPath"}, {"kind":"literal","value":"example.invalid"}, {"kind":"literal","value":"--host"}]);
...
fn nested_unknowns_ids_paths_and_execution_context_fail_closed() {
    ...
    for (path, value) in [
        ...
        ("/launch/interpreter/artifact/sha256", json!("latest")),
