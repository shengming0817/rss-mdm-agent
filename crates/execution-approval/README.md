...
let decision = execution_approval::evaluate(
    &frozen_plan, &admission_decision, &profile_bindings,
...
attempt 直接继承 C07，入口没有可替换的 attempt 参数。NotRequired 与 Satisfied 均保留 C07 的私有 AdmissionValidity（可信主体/委托/政策/撤销 revision、验证时间、排他截止）；无批准要求也不能跳过授权新鲜度。批准验证时间越过该窗口返回 StaleAdmission，消费意图的截止同时受该窗口约束。
...
1. 核对 authority/plan/attempt 与批准决定、生命周期 BeginAttempt 候选的绑定；调用 valid_for_commit，以事务内的当前授权 revision 和可靠时间验证 C07 身份与截止，并重检批准的 trust/revocation revision。
2. 校验事件/attempt 历史唯一性、生命周期 revision、每个记录版本与消费计数 CAS。
3. 通过生命周期 Transition::commit 的可信回调原子提交全部消费、执行 intent、快照和裁决审计；仅首次提交返回 Applied 并取得一次性 DispatchAction。同 attempt 重放返回 AlreadyCommitted，失败、冲突或结果不明返回错误，都不产生派发动作。
...
上限测试覆盖零值、profile/binding/不同 record 超限且 verifier 未调用，以及恰好等于上限的成功路径。Cargo manifest 为本包全部 targets 启用认知复杂度 lint，普通 clippy 包含集成测试。
